...
(setq-default
 ...
 frame-title-format '((:eval (projectile-project-name))) ; Change frame title to project name
...
(use-package spaceline
  ...
  (spaceline-define-segment
    ...
    (concat
     "Twitter "
     (if (boundp 'twittering-unread-status-info)
         (powerline-raw (twittering-make-unread-status-notifier-string))
       "0")))
  :config
  (setq-default
  ...
  (spaceline-install
    '((major-mode :face highlight-face)
      ((elnawe/version-control buffer-id) :separator " ")
      (line-column))
    '((anzu :when active)
...
(setq
 ...
 calendar-longitude -60.6
 calendar-location-name "Rosario, Argentina")
#+END_SRC
