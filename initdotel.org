...
I'm a pretty big fan of Terminal Operating System and my desktop (and one of my
laptops) has a clean ArchLinux installed with no visual desktop o GUI. These
configurations are needed to ensure that my Emacs works well on those boxes!
...
When I am updating or changing some configuration on my Emacs I like to restart
it to clean up everything I removed. There's an excellent package to do that and
it's called =restart-emacs=. Instead of =C-x C-c= (quit-emacs) I use =C-x C-M-c=
to restart it.
...
(use-package zenburn-theme
  ...
  (zenburn-with-color-variables
    ...
    (set-face-attribute 'vertical-border nil :foreground zenburn-yellow-2))
...
I tend to use the terminal a lot and most of the programs you use there uses a
vi-like navigation. You just get used to it.
...
*Note*: Weather icons [[Forecast][here]]
...
=w3m= is one of the best browser out there and it's even more useful when you
only work on =TOS= machines, this segments shows the title of the website I'm
...
First I used =helm= but it felt slow. Then I moved to the built-in =ido-mode=
and, though it worked great, I wanted to try =ivy= and after searching the
...
Since I use Emacs on my =TOS= a lot and sometimes need to browse on the web, I
need a powerful tool and =w3m= is pretty awesome!
