...
(setq nemacs-org-archive-file (nemacs-get-org-file "archive")
      nemacs-org-calendar-file (expand-file-name "my-calendar.org" "~/Notes/references/calendar")
...
(setq
 org-agenda-files (list nemacs-org-inbox-file nemacs-org-calendar-file)
...
(setq org-todo-keywords
      '((sequence "TODO(t)" "CURRENT(y)" "WAITING(w@)" "|" "DONE(d)" "CANCELED(c)"))
...
Create an =ID= when using the =org-store-link=. This changes the behavior of traditional links in Org so if I refile an
entry, the link doesn't break.
