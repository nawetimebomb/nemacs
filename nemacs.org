...
(defun nemacs-add-created-property-to-capture ()
  "Adds a `:created:' property inside the `:PROPERTIES:' drawer."
  (interactive)
  (org-set-property "created" (concat "<" (format-time-string "%F %a %H:%M") ">")))
#+END_SRC
...
(use-package org
  ...
  :config
  ;; Variables
  (setq nemacs-org-journal-file (nemacs-get-org-file "journal")
  ...
  ;; Agenda
  (setq
   org-agenda-files (list nemacs-org-inbox-file nemacs-org-journal-file)
  ...
  (setq org-todo-keywords
        '((sequence "TODO" "INPROGRESS" "|" "DONE")
          (list "URGENT" "CANCELED" "MEETING" "EMACS"))
        org-todo-keyword-faces nil
        org-capture-templates '(("t" "Add a Todo"
                                 entry (file+headline nemacs-org-inbox-file "Tasks")
                                 "** TODO %?" :kill-buffer t)
                                ("m" "Add meeting notes"
                                 entry (file+headline nemacs-org-inbox-file "Meetings")
                                 "** MEETING %^{Title} %^{attendees}p %^{location}p \n%?" :jump-to-captured t)
                                ("e" "Add Emacs-related note"
                                 entry (file+headline nemacs-org-inbox-file "My Emacs")
                                 "** EMACS %?" :kill-buffer t)
                                ("j" "Add entry to the journal"
                                 item (file+datetree nemacs-org-journal-file)
                                 "%?" :kill-buffer t))
        org-agenda-custom-commands '(("n" "Agenda and all Todo's"
                                      ((agenda "")
                                       (alltodo "")))))
    (add-hook 'org-capture-before-finalize-hook #'nemacs-add-created-property-to-capture))
