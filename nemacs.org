...
#+BEGIN_SRC emacs-lisp
(defvar nemacs-secrets-file (expand-file-name "secrets.el" "~/elisp"))

(when (file-exists-p nemacs-secrets-file)
  (load nemacs-secrets-file))
#+END_SRC

** Agenda

I've been using Emacs for almost one year now. I learned that doing things by yourself is the best way to go and
although I'm still using some packages, I'm trying to configure things my way. There's a lot of custom code below and
I'm proud of it!

*** Helpers

A couple of helper functions and variables to organize my agenda easily.
...
(setq nemacs-org-archive-file (nemacs-get-org-file "archive.org")
      nemacs-org-calendar-file (nemacs-get-org-file "references/calendar/my-calendar.org")
...
*** Prompt Capture Setup

Default =org-capture= is very good, but I want to integrate my =capture= with other things so I don't need to
configure unnecessary keybindings. This code below is a customized copy of =org-mks= with a personal touch to run any
lambda function. *Warning:* Lisp is contagious.

#+BEGIN_SRC emacs-lisp
(defvar nemacs-capture-templates '()
  "List of possible capture keys.
It's automatically filled with `org-capture-templates' but can append more values.
Expects a triplet with `key', `description' and `a callback function'.")
...
(defun nemacs-capture (&optional keys)
  "Interactive function similar to `org-capture' but with a personal touch.
Launch the `nemacs-create-prompt-buffer' function and execute the `callback' of the selection."
  (interactive)
  (let ((selected-action
         (nemacs-create-prompt-buffer nemacs-capture-templates
                               "NEMACS CAPTURE\n==============\nSelect a capture template:"
                               "Template key: ")))
    (funcall (car (cddr selected-action)))))
#+END_SRC

*** Configuration

Main configuration for my Agenda. Starting with the keybindings.

#+BEGIN_SRC emacs-lisp
(global-set-key (kbd "C-c a") #'org-agenda)
(global-set-key (kbd "C-c c") #'nemacs-capture)
(global-set-key (kbd "C-c d") #'nemacs-dashboard)
(global-set-key (kbd "C-c l") #'org-store-link)

(define-key org-mode-map [remap org-archive-subtree-default] #'nemacs-org-mark-done-and-archive)
#+END_SRC

Now the big settings. I separated those by different headers which are explained below.

- Defaults: A list of variables with better defaults for my setup.
- Capture: The capture configuration with better =org-todo= keys and templates.
- Refile: A bunch of configurations to the refiling.
- Tags: Better tags with completition for context.

#+BEGIN_SRC emacs-lisp
(require 'org-id)

;; Defaults
(setq org-agenda-files (list nemacs-org-inbox-file nemacs-org-calendar-file)
...
Finally, populating the =nemacs-capture-templates= with the =org-capture-templates=.

#+BEGIN_SRC emacs-lisp
(dolist (element (reverse org-capture-templates))
  (let ((key (car element))
        (description (car (cdr element))))
    (setq-local capture-entry (list key description `(lambda () (org-capture :keys ,key))))
    (when (not (member capture-entry nemacs-capture-templates))
      (push
        capture-entry nemacs-capture-templates))))
#+END_SRC

*** Org Bullets

Adding the =org-bullets= package to replace the traditional * with a fancy-looking ASCII bullet. I might replace this
with a custom function in the future.
...
Other =org= related configuration that is not related to the Agenda. Here I'm setting up some defaults and configuring
other org-related packages that I use.
...
My main configuration file uses =org-mode= and I usually use this code inside the text feature for emails and more.
...
Accessing JIRA from Org makes JIRA better.
...
(use-package org-jira
  ...
  :config
  (setq org-jira-working-dir nemacs-cache-dir))
#+END_SRC

*** Org Calendar

I have my calendar synced with Org mode. However, the configuration is being loaded from a =secrets.el= which is shared
between selected systems.
