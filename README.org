...
| custom        | Folder | Custom files that are not promoted to the version control system. Modifications are local to that system        |
| init.el       | File   | Initialization file                                                                                             |
| early-init.el | File   | Loaded before the init file. Modifies the initial Emacs configuration, like disabling the original =package.el= |
